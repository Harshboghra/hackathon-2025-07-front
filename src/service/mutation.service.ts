...
export const MUTATION_TYPE_DUPLICATE = 'duplicate';
export const MUTATION_TYPE_MERGE = 'merge';
export const MUTATION_TYPE_VALIDATE = 'validate';
...
export const mutateFromFormData = (
  ...
  const mutation = buildMutationFromJson(
    formData,
    objectName,
    type,
    responseAttribute,
...
export const handelMutationResponse = async (response: any) => {
  ...
  if (Object.keys(res).includes('errors')) {
    ...
    return {
      success: (Object.values(newResponse)[0] as any).response
        ?.success as boolean,
